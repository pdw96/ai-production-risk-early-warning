...
# 속성 확장 표가 붙는 그룹 셋. 나머지 그룹은 속성이 없어 공통코드만으로 끝난다.
#
# 공통코드 한 표에 속성을 다 밀어 넣으면 대부분이 빈 칸이 되고 실무에서 흔한
# `attr1 … attr9` 로 끝난다 — 「attr3이 무슨 뜻인지 아무도 모른다」가 그 결말이다.
# 그래서 두 층으로 가른다: 본체에는 모든 분류가 함께 쓰는 것만 두고(코드 · 명칭 ·
# 정렬 · 사용 여부 · 설명), 코드마다 딸린 속성은 그 코드를 참조하는 작은 표에 둔다.
GROUPS_WITH_ATTRIBUTES: tuple[str, ...] = (TXN_TYPE, NC_REASON, PO_CLOSE)
