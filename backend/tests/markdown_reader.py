...
def prose_lines(text: str) -> list[str]:
    ...
    **안 보는 것 — 탭.** CommonMark 는 탭을 **4열 정지**로 펼치지만 여기서는 줄을
    글자로 센다(`lstrip(" ")` 는 탭을 들여쓰기로 세지도 않는다). 탭이 어디 있느냐에
    따라 **두 방향으로 다 새므로** 한 줄로 「탭은 안 본다」고만 적지 않는다.

    - **줄머리가 탭이면 거짓 초록.** 들여쓰기가 0으로 읽혀 코드가 산문이 된다 —
      탭 하나로 들여쓴 링크는 화면에서 코드 블록인데 링크로 세어진다.
    - **목록 마커 뒤가 탭이면 거짓 빨강.** 내용 열을 얕게 잡아 이어지는 줄을 코드로
      지운다 — `-` 다음이 탭이면 내용 열이 4(코드는 8칸부터)인데 2(코드 6칸)로
      읽혀, 6칸 줄이 산문인데 사라진다.

    닫지 않은 것은 줄을 글자가 아니라 **열**로 다시 세야 해서다 — 머리말의 기준에서
    설계를 깨는 쪽이다. 다만 **거짓 빨강이 섞여 있다는 것을 같이 적는다.** 그쪽은
    선을 긋고 둘 성질이 아니므로, 이 저장소의 문서가 실제로 탭을 쓰기 시작하면 여기가
    먼저 다시 볼 자리다.
    """
    out: list[str] = []
...
def _code_column(previous_content: str | None) -> int:
    ...
    **마커 뒤가 탭이면 이 계산이 어긋난다** — 한계와 그 방향은 `prose_lines` 가
    적는다. 두 곳에 적으면 한쪽이 조용히 낡기 때문에 여기서는 가리키기만 한다.
