...
**아직 스스로 엔진을 만드는 검사가 남아 있다.** 옮길지는 **묻는 것이 무엇인가**로
가른다 — 수를 세어 정하지 않는다.

- **묻는 것이 엔진이면 옮긴다.** 값·제약·질의가 엔진에 따라 달라질 수 있으면 공용
  픽스처로 바꿔 단다.
- **묻는 것이 엔진이 아니면 둔다.** 떨어지는 경로나 하위 프로세스 기동 **자체가 물음**인
  검사는 설정된 엔진으로 옮기는 순간 **묻는 내용이 바뀐다.**
- **관찰 수단이 SQLite 전용이라는 것만으로는 「못 옮김」이 아니다.** `sqlite_master` 를
  읽더라도 묻는 것이 그 카탈로그가 아니라 **reset 이 무엇을 지우고 무엇을 남기는가**
  라면, 관찰을 `sa.inspect` 로 바꾸면 물음은 그대로다. 수단과 물음을 혼동하면 엔진에
  따라 달라질 수 있는 동작이 계속 한 엔진에서만 검증된다.

어디에 남아 있는지는 `grep -rn create_engine backend/tests/*.py` 가 짚어 준다. **수를
여기 적지 않는 것은 그것이 상태이기 때문이다** — 옮길 때마다 낡고, 낡은 줄 모른 채
다음 사람의 전제가 된다.
