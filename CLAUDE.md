...
**엔진을 묻지 않는 검사는 옮기지 않는다.** 묻는 것이 엔진이 아니라 `sqlite_master` 의
내용이거나 떨어지는 경로이거나 하위 프로세스의 기동이면, 설정된 엔진으로 옮기는 순간
**묻는 내용 자체**가 바뀐다. 그런 검사는 그대로 둔다.

**무엇이 그것인지는 위 「스스로 엔진을 만들어…」 문단 한 곳에만 적는다.** 여기서 다시
세지 않는 것은 두 곳에 적으면 한쪽을 고칠 때 나머지가 조용히 거짓이 되기 때문이다 —
실제로 그렇게 됐다가 고쳤다. **옮기지 않은 것과 못 옮긴 것을 가르는 것도 그 문단이
한다**; 여기서 「남은 것은 다 못 옮긴다」로 뭉뚱그리면 옮길 수 있는 것까지 손대면 안
되는 것으로 굳는다.
