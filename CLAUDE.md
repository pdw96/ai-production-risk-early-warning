...
있으면 공용 픽스처로 옮긴다. 옮기는 순간 묻는 내용 자체가 달라지면 둔다 —
옮기지 않은 것과 못 옮긴 것은 다르다. **관찰 수단이 한 엔진 전용인 것은 두는
이유가 아니다.** 수단은 바꿀 수 있다 — `sqlite_master` 를 직접 읽던 자리는
`sa.inspect(engine).get_table_names()` 로 바꾸어 묻는 내용을 그대로 둔 채 옮겼다.
아직 스스로 엔진을 만드는 자리는 `grep -rn create_engine backend/tests/*.py` 가
짚어 준다.
