...
- **스스로 다음 일을 집지 않는다.** 이슈가 열려 있다는 것은 「지금 할 차례」가
  아니라 **「하기로 했다」**이다. 무엇을 언제 집을지는 저자가 정한다 — 마일스톤이
  다 닫혀도 마찬가지다(`#48`).
...
## 되풀이되는 규칙은 `.claude/skills/` 에 있다

이 파일은 **저장소**를 어떻게 다루는지를 적는다. 그런데 이 저장소에서 일하려면
저장소 밖의 것도 알아야 한다 — 리뷰 봇이 판정을 어떤 모양으로 남기는지, 무엇이
막히고 막혔을 때 무엇을 의심하는지, 무엇을 사람에게 남기는지. **그것이 발화로
전달되면 레인을 띄울 때마다 사람이 실어 나른다.**

그래서 `.claude/skills/` 에 내려 둔다. **목록을 여기 적지 않는다** — 스킬이 늘거나
이름이 바뀌면 이 줄이 조용히 낡고, 그 짝을 보는 기계가 없기 때문이다. 각 스킬의
머리말이 **언제 그것을 보는지**를 스스로 적으므로 `ls .claude/skills` 로 열어 본다.

**여기와 스킬이 어긋나면 「절대 하지 않는 것」이 권위다.** 스킬은 그 금지를 다시
쓰지 않고, 그것이 답하지 않는 것만 든다.
...
1. 이 파일.
2. `.claude/skills/` — 저장소 밖의 되풀이되는 규칙이 거기 있다. 머리말만 훑고,
   해당하는 것을 열어 본다.
3. 열린 이슈와 마일스톤 — **하기로 정해진 것과 지금 상태**가 거기 있다. 무엇을
   먼저 집을지는 **여기서 읽지 않는다** — 위의 「스스로 다음 일을 집지 않는다」대로
   저자가 고른다. 허브는 왜 그것을 하는지를 적는다.
4. `git fetch origin main` **먼저**, 그 다음 `git log --oneline -10 origin/main` 과
   열린 PR. 당겨오지 않으면 **다른 레인이 머지한 것을 못 본 채로 시작한다.**
5. `CHECKLIST.md` — **PR 을 내기 전에** 본다. 거기 해당하는 절을 복사해 PR 에서
   채운다.
