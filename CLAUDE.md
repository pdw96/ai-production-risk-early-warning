...
그래서 `.claude/skills/` 에 내려 둔다. 각 스킬의 머리말이 **언제 그것을 보는지**를
적고 있으므로, 목록을 여기 베끼지 않는다 — `ls .claude/skills` 가 지금을 든다.

- **`pr-round`** — PR 을 열고 리뷰 한 라운드를 읽는다. 「지적이 없다」를
  「깨끗하다」로 읽기 전에 본다
- **`env-reach`** — 무엇이 닿고 무엇이 막히는가. **막힘은 규칙이 아니라 측정이라**
  잰 날짜가 붙어 있다
- **`authority-line`** — 묻지 않고 하는 것과 사람에게 남기는 것의 선
- **`durable-writing`** — 낡지 않는 글을 쓰는 법. 이슈 본문 네 줄 규약이 여기 있다
...
2. `.claude/skills/` — 저장소 밖의 되풀이되는 규칙이 거기 있다. 머리말만 훑고,
   해당하는 것을 열어 본다.
3. 열린 이슈와 마일스톤 — 지금 무엇을 할 차례인지는 거기 있다. 허브는 왜 그것을 하는지를 적는다.
