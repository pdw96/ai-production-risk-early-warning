...
**아직 스스로 엔진을 만드는 곳이 둘 남아 있고, 그것은 그대로 두는 것이 맞다.**
`test_seed.py` 의 12개(아래 「엔진을 묻지 않는 검사」)와 `test_migrations.py` 다.
뒤엣것은 `create_all` 로 세운 표와 마이그레이션으로 세운 표가 같은지를 견주므로
**두 스키마를 나란히 놓을 데이터베이스 둘**이 필요하고, 그 비교 자체는 방언에
매이지 않는다. 남은 것과 옮긴 것을 세어 보고 싶으면
`grep -rn create_engine backend/tests/*.py` 가 지금 상태를 그대로 보여 준다.
...
프로세스로 돌리는 검사가 남아 있다(35개 중 12개). 그것들을 설정된 엔진으로
옮기려면 **묻는 내용 자체**를 바꿔야 하므로 그대로 둔다 — 옮기지 않은 것과 못
옮긴 것은 다르다.
