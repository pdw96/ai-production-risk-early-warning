...
이 저장소는 **두 가지를 동시에** 만듭니다. 섞어 읽으면 「2단계」 같은 말이 두 뜻으로 서므로, 이름을 갈라 둡니다.

**제품** — 화면과 판정이 있는 쪽입니다.

1. **제품 1단계 — ERP Setup.** 기준정보·구매·재고·생산·영업·품질이 한 벌로 도는 ERP 를 세웁니다. 아래 [화면 안내](#화면-안내)가 그 결과입니다.
2. **제품 2단계 — ERP 기반 리스크 조기 탐지 Setup.** 그 ERP 의 값에서 납기·자재 위험을 계산해 조기에 띄웁니다. [판정 규칙](#판정-규칙)이 그 자리입니다.

**파이프라인** — 제품을 만드는 데 쓰는 규칙·검사·자동화입니다. 마일스톤 `P0`·`P1`·`P2` 가 그쪽이고, 각 마일스톤 설명에 어느 축인지와 무엇을 받치는지가 적혀 있습니다.

**어디까지 왔는지는 이 파일이 들지 않습니다** — 손으로 적은 진척은 반드시 낡습니다. [열린 이슈](../../issues)와 [마일스톤](../../milestones)이 셉니다. 다만 마일스톤이 세는 것은 **파이프라인 쪽뿐**입니다.
