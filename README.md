...
`materials`에는 재고 수량 컬럼이 없습니다. **자재 재고는 `material_lots`의 합계에서 파생됩니다.** 로트는 로트번호, 보관 창고(`원재료창고` / `생산창고`), 수량, 입고일, 유효기간을 가집니다. 원재료창고에 있던 한 로트의 일부를 생산창고로 옮긴 상태를 표현해야 하므로 **로트의 유일키는 로트번호 단독이 아니라 `(자재, 로트번호, 창고)` 조합**입니다(로트번호는 자재에 종속된 개념이라 자재까지 넣습니다). 보관 창고는 이 두 값만 저장되도록 DB 제약으로 막습니다. 예정 입고(`purchase_receipts`)도 도착하면 로트가 되므로 유효기간을 가지며, 도착지는 원재료창고입니다. 화면의 `가용 재고`는 **기준일에 도착했고 만료되지 않은 로트의 합**이라 `material_lots` 행 합계와 다를 수 있습니다 — 입고를 수요 차감보다 먼저 반영하는 규칙 때문에 **기준일 당일 도착하는 예정 입고분이 포함**되기 때문입니다. 응답의 로트 목록에는 14일 내 예정 입고와 이미 만료된 로트(`만료` 상태)도 함께 들어 있으므로 목록 전체 합계도 `가용 재고`와 다릅니다. **입고일이 기준일 이전이고 유효기간이 기준일보다 뒤인 행만** 더해야 일치합니다(회귀 테스트: `test_available_stock_equals_the_sum_of_received_and_unexpired_lots`).
