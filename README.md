...
14일 안에 안전재고 미만이 되거나 재고가 0 이하가 되면 부족 예상으로 보고 권장 조치를 표시합니다. **소진일(`stockout_date`)은 재고가 실제로 0 이하가 되는 경우에만 붙습니다**(`app/services/material_risk.py`). 안전재고 미만으로만 내려가는 경우의 근거는 두 갈래입니다(`app/services/briefing.py`) — 부족의 원인이 유효기간 폐기라면 **폐기 수량과 부족이 시작되는 날**을 함께 적고, 그렇지 않으면 「안전재고 미만으로 하락」이라고만 적습니다. 권장 조치도 그 갈래를 따릅니다. 충족하지 못한 수요는 다음 날로 이월되며, 뒤늦게 도착한 입고가 그 적자를 먼저 갚습니다.
