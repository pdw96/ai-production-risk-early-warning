...
`products`(가상 제품 5개), `orders`(생산 오더 30개), `daily_productions`(과거 30일 이상 및 향후 14일 계획), `materials`(자재 15개), `material_lots`(자재의 로트별 보유 재고), `bom_requirements`(제품-자재 단위 소요량), `purchase_receipts`(예정 입고), `risk_statuses`(결정적 리스크 키와 상태)를 사용합니다.

`materials`에는 재고 수량 컬럼이 없습니다. **자재 재고는 `material_lots`의 합계에서 파생됩니다.** 로트는 로트번호, 보관 창고(`원재료창고` / `생산창고`), 수량, 입고일, 유효기간을 가집니다. 원재료창고에 있던 한 로트의 일부를 생산창고로 옮긴 상태를 표현해야 하므로 **로트의 유일키는 로트번호 단독이 아니라 `(자재, 로트번호, 창고)` 조합**입니다(로트번호는 자재에 종속된 개념이라 자재까지 넣습니다). 보관 창고는 이 두 값만 저장되도록 DB 제약으로 막습니다. 예정 입고(`purchase_receipts`)도 도착하면 로트가 되므로 유효기간을 가지며, 도착지는 원재료창고입니다. 화면의 `가용 재고`는 **기준일에 도착했고 만료되지 않은 로트의 합**이라 `material_lots` 행 합계와 다를 수 있습니다 — 입고를 수요 차감보다 먼저 반영하는 규칙 때문에 **기준일 당일 도착하는 예정 입고분이 포함**되기 때문입니다. 응답의 로트 목록에는 14일 내 예정 입고와 이미 만료된 로트(`만료` 상태)도 함께 들어 있으므로 목록 전체 합계도 `가용 재고`와 다릅니다. **`입고일 <= 기준일` 이고 `유효기간 없음 OR 유효기간 > 기준일` 인 행만** 더해야 일치합니다(기준일 당일 입고분과 유효기간이 없는 로트가 모두 가용에 들어갑니다)(회귀 테스트: `test_available_stock_equals_the_sum_of_received_and_unexpired_lots`).
...
완제품 창고는 이 모델에 없습니다. 완제품은 자재가 아니라 제품이고, 완제품 로트와 출하 리스크는 아래 [향후 확장 계획](#향후-확장-계획)의 별도 항목입니다.
...
> **LOT/FIFO 도입으로 스키마가 바뀌었습니다.** 이 저장소에는 마이그레이션 도구가 없으므로 **이전 버전에서 만든 DB는 재시드해야 합니다.** 로컬은 `python -m app.seed`, 도커는 `docker compose down -v` 로 볼륨을 지우고 다시 올립니다. **기록해 둔 리스크 상태는 사라집니다.**
...
GitHub Actions의 `Cloud validation` 워크플로는 테스트·타입 검사·빌드를 수행하고 FastAPI와 Next.js를 일시적으로 실행합니다. 완료된 실행의 **Artifacts → production-risk-screenshots**에서 운영 현황(`dashboard.png`), 생산관리(`orders.png`), 재고관리(`materials.png`), 리스크 보드(`risks.png`), 기준정보관리(`master.png`), 구매관리(`purchases.png`) 화면 PNG를 내려받을 수 있습니다. 영업관리는 준비 중인 정적 화면이라 촬영하지 않습니다. `workflow_dispatch`가 설정되어 있으므로 Actions 화면에서 수동으로도 실행할 수 있습니다.
...
| ERP | 재고관리 | `/materials` | 로트별 재고와 창고, 14일 수급·안전재고·소진 예상일 |
| ERP | 생산관리 | `/orders` | 생산 오더 납기 현황, 최근 14일 생산실적, 오더 상세(`/orders/{id}`) |
| ERP | 영업관리 | `/sales` | **준비 중** — 출하·완제품 재고 엔티티가 아직 없습니다 |

**경로는 개편 전 그대로 둡니다.** 생산관리가 `/orders`, 재고관리가 `/materials`인 것은 메뉴 라벨만 업무 모듈 이름으로 바꾸고 URL은 유지해 기존 링크와 CI 스크린샷 경로가 깨지지 않게 했기 때문입니다.

영업관리는 자리만 잡아 둔 빈 모듈입니다. 완제품 로트와 출하 일정이 없으면 부족을 판정할 수 없어, 아래 [향후 확장 계획](#향후-확장-계획)의 완제품·출하 항목이 선행돼야 합니다.
...
**LOT/FIFO**는 구현되어 위 [14일 안전재고 — LOT/FIFO 기반](#14일-안전재고--lotfifo-기반)에 반영되었습니다. 남은 계획은 다음과 같습니다.

- **재고이동 처리**: 창고 간 재고이동을 행위로 기록하고 이력을 남깁니다. 현재 앱의 쓰기 경로는 리스크 상태 변경 하나뿐이라, 이는 기능 추가가 아니라 쓰기 워크플로 도입입니다.
- **완제품 로트·완제품 창고**: 생산된 완제품에 로트를 부여하고 완제품 창고 재고를 관리합니다. 완제품은 자재가 아니라 제품이므로 새 엔티티와 생산 실적↔로트 연결이 필요합니다.
- **출하 리스크**: 출하 일정과 완제품 재고를 비교해 부족을 경보합니다. 납기·자재에 이은 세 번째 리스크 타입이며, 기존 납기 리스크와 같은 사건을 두 번 경보할 수 있어 통합 설계가 선행돼야 합니다.
