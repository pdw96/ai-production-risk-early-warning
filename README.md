...
`materials`에는 재고 수량 컬럼이 없습니다. **자재 재고는 `material_lots`의 합계에서 파생됩니다.** 로트는 로트번호, 보관 창고(`원재료창고` / `생산창고`), 수량, 입고일, 유효기간을 가집니다. 원재료창고에 있던 한 로트의 일부를 생산창고로 옮긴 상태를 표현해야 하므로 **로트의 유일키는 로트번호 단독이 아니라 `(로트번호, 창고)` 조합**입니다. 예정 입고(`purchase_receipts`)도 도착하면 로트가 되므로 유효기간을 가지며, 도착지는 원재료창고입니다.
