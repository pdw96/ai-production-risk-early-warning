...
`materials`에는 재고 수량 컬럼이 없습니다. **자재 재고는 `material_lots`의 합계에서 파생됩니다.** 로트는 로트번호, 보관 창고(`원재료창고` / `생산창고`), 수량, 입고일, 유효기간을 가집니다. 원재료창고에 있던 한 로트의 일부를 생산창고로 옮긴 상태를 표현해야 하므로 **로트의 유일키는 로트번호 단독이 아니라 `(자재, 로트번호, 창고)` 조합**입니다(로트번호는 자재에 종속된 개념이라 자재까지 넣습니다). 보관 창고는 이 두 값만 저장되도록 DB 제약으로 막습니다. 창고는 모두 세 곳(`원재료창고`·`생산창고`·`완제품창고`)이지만 **자재가 쓰는 목록과 완제품이 쓰는 목록은 다릅니다** — 자재는 `원재료창고`/`생산창고`, 완제품은 `생산창고`/`완제품창고`입니다. 생산창고가 양쪽에 드는 것은 라인 옆에 투입 대기 자재와 갓 생산된 완제품이 같이 있기 때문이며, 목록을 하나로 합치면 자재를 완제품창고에 넣는 것을 막을 수 없습니다. 예정 입고(`purchase_receipts`)도 도착하면 로트가 되므로 유효기간을 가지며, 도착지는 원재료창고입니다. 화면의 `가용 재고`는 **기준일에 도착했고 만료되지 않은 로트의 합**이라 `material_lots` 행 합계와 다를 수 있습니다 — 입고를 수요 차감보다 먼저 반영하는 규칙 때문에 **기준일 당일 도착하는 예정 입고분이 포함**되기 때문입니다. 응답의 로트 목록에는 14일 내 예정 입고와 이미 만료된 로트(`만료` 상태)도 함께 들어 있으므로 목록 전체 합계도 `가용 재고`와 다릅니다. **`입고일 <= 기준일` 이고 `유효기간 없음 OR 유효기간 > 기준일` 인 행만** 더해야 일치합니다(기준일 당일 입고분과 유효기간이 없는 로트가 모두 가용에 들어갑니다)(회귀 테스트: `test_available_stock_equals_the_sum_of_received_and_unexpired_lots`).
...
완제품창고 재고는 이 계산에 들어가지 않습니다. 완제품은 자재가 아니라 제품이고, 14일 전망이 다루는 것은 생산에 투입되는 자재이기 때문입니다. 완제품 재고는 아래 [완제품 로트와 출하검사](#완제품-로트와-출하검사)에서 따로 다룹니다.
...
갓 생산된 완제품은 생산창고에서 출하검사(OQC)를 기다립니다. **합격분만 완제품창고로 이관되고, 출하는 완제품창고 재고에 한해 일어납니다.** 불합격분은 생산창고에 남습니다. 완제품창고에 검사 대기·불합격이 섞이면 출하 가능 수량이 실제보다 많아 보이므로, `완제품창고에 있으려면 OQC 합격이어야 한다`를 DB CHECK 제약으로 막습니다. 반대 방향은 강제하지 않습니다 — 합격이 곧 이동은 아니어서 `이관 대기` 상태가 실제로 존재합니다.

영업관리 화면의 다섯 수량은 서로 겹치지 않으며 합이 로트 행 합계와 같습니다.

| 수량 | 정의 |
| --- | --- |
| 출하 가능 | 완제품창고 + 미만료 |
| 이관 대기 | OQC 합격이지만 아직 생산창고 |
| 검사 대기 | 아직 OQC를 받지 않음 |
| 불합격 | OQC 불합격 |
...
> **완제품 로트·품질관리 도입으로 스키마가 또 바뀌었습니다.** 완제품 로트(`finished_goods_lots`)와 검사 기록(`quality_inspections`) 테이블이 생겼고, 제품·자재에 유효기간 설정기간(`shelf_life_days`)이 붙었습니다. 이 저장소에는 마이그레이션 도구가 없으므로 **이전 버전에서 만든 DB는 재시드해야 합니다.** 로컬은 `python -m app.seed`, 도커는 `docker compose down -v` 로 볼륨을 지우고 다시 올립니다. **기록해 둔 리스크 상태는 사라집니다.**
...
| ERP | 영업관리 | `/sales` | 제품별 완제품 로트 재고, 출하 가능·이관 대기·검사 대기·불합격·만료 구분 |

**경로는 개편 전 그대로 둡니다.** 생산관리가 `/orders`, 재고관리가 `/materials`인 것은 메뉴 라벨만 업무 모듈 이름으로 바꾸고 URL은 유지해 기존 링크와 CI 스크린샷 경로가 깨지지 않게 했기 때문입니다. 품질관리만 신규 화면이라 `/quality`를 새로 씁니다. 메뉴에서 생산관리와 영업관리 사이에 둔 것은 자재 입고(IQC) → 생산(PQC) → 출하(OQC) 순서이고, OQC가 출하의 관문이기 때문입니다.

영업관리는 완제품 재고까지만 다룹니다. 출하 일정과 수주 엔티티가 없어 **부족 판정은 아직 하지 않습니다** — 자세한 내용은 위 [완제품 로트와 출하검사](#완제품-로트와-출하검사)에 있습니다.
...
**LOT/FIFO**는 위 [14일 안전재고 — LOT/FIFO 기반](#14일-안전재고--lotfifo-기반)에, **완제품 로트·완제품창고**와 **품질관리(IQC·PQC·OQC)** 는 위 [완제품 로트와 출하검사](#완제품-로트와-출하검사)에 구현되어 있습니다. 남은 계획을 우선순위 순으로 적습니다.

1. **출하 리스크**: 출하 일정과 완제품 재고를 비교해 부족을 경보합니다. 납기·자재에 이은 세 번째 리스크 타입이며, 기존 납기 리스크와 같은 사건을 두 번 경보할 수 있어 통합 설계가 선행돼야 합니다. 차감은 완제품창고의 OQC 합격 로트에서만 일어납니다.
2. **재고이동 처리**: 창고 간 재고이동을 행위로 기록하고 이력을 남깁니다. 현재 앱의 쓰기 경로는 리스크 상태 변경 하나뿐이라, 이는 기능 추가가 아니라 쓰기 워크플로 도입입니다. 완제품의 `이관 대기 → 완제품창고`, 자재의 `원재료창고 → 생산창고`가 그 대상입니다.
