...
14일 안에 안전재고 미만이 되거나 재고가 0 이하가 되면 부족 예상으로 보고 권장 조치를 표시합니다. **소진일(`stockout_date`)은 재고가 실제로 0 이하가 되는 경우에만 붙습니다** — 안전재고 미만으로만 내려가는 경우는 근거가 「안전재고 미만으로 하락」이라고만 적힙니다(`app/services/material_risk.py`). 부족의 원인이 유효기간 폐기라면 판정 근거와 권장 조치가 그 사실을 가리킵니다. 충족하지 못한 수요는 다음 날로 이월되며, 뒤늦게 도착한 입고가 그 적자를 먼저 갚습니다.
