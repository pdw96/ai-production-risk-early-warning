...
4. **`available_stock` 은 기준일에 도착하는 예정 입고를 포함한다.** 입고를
   ...
   `material_lots` 행 합계와 화면의 "가용 재고" 가 다를 수 있다. 응답의 로트
   목록도 그냥 다 더하면 안 된다 — 14일 내 예정 입고와 이미 만료된 로트
   (`만료` 상태)가 함께 들어 있기 때문이다. 처음 쓴 테스트가 이 차이를 몰라
   실패했고, 정확한 불변식(`입고일 <= 기준일` 이고
   `유효기간 없음 OR 유효기간 > 기준일` 인 행의 합 == 가용 재고)으로 다시
   썼다. 기준일 당일 입고분과 유효기간이 없는 로트가 모두 가용에 들어간다. 2026-09-02 리뷰에서 README 가 "목록
   합계와는 일치한다"고 느슨하게 적어 둔 것이 지적돼 이 필터를 명시했다.
