...
redact_url() {
  ...
  #
  # **사용자 이름 쪽도 마찬가지였다.** `[^:/@]*` 는 `@` 가 든 이름을 막는데, 파서는
  # **마지막 `@`** 로 사용자 정보와 호스트를 가른다. 실측(2026-09-09):
  #
  #   make_url("postgresql+psycopg://a@b:secret@host/db")
  #     → username='a@b' password='secret' host='host'
  #
  # 그 주소도 동작하는 주소이고, 옛 묶음은 첫 `@` 에서 막혀 아무것도 가리지 못했다.
  #
  # **사용자 이름 쪽을 `.*` 로 열어 둔 것은 틀렸다.** 탐욕적인 `.*` 는 `/` 도 `?` 도
  # 넘으므로 **질의 문자열 안까지** 짚어 간다. 실측(2026-09-09):
  #
  #   ://u:secret@host/db?application_name=a:b@c
  #     → ://u:secret@host/db?application_name=a:***@c
  #
  # 진짜 비밀번호 `secret` 이 로그에 통째로 박히고 질의값 `b` 가 대신 가려졌다.
  # `make_url` 은 그 주소의 비밀번호를 `secret` 으로 읽는다(실측) — 즉 동작하는
  # 주소이고, 새 셸마다 도는 훅이 그것을 찍는다.
  #
  # **짐작을 그만두고 파서의 문법을 그대로 쓴다.** SQLAlchemy 2.0.52 의
  # `sqlalchemy.engine.url` 이 쓰는 정규식은 사용자 정보를 이렇게 가른다:
  #
  #   (?:(?P<username>[^:/]*)(?::(?P<password>[^@]*))?@)?
  #
  # 사용자 이름은 `@` 는 담되 `:` 와 `/` 는 담지 못하고, 비밀번호는 `@` 를 담지
  # 못한다. 그래서 여기도 `[^:/]*` 와 `[^@]*` 다 — 한 칸도 넓지도 좁지도 않다.
  # `[^:/]*` 는 `/` 를 넘지 못하므로 경로에도 질의에도 닿지 않고, `@` 는 담으므로
  # `a@b` 같은 이름도 파서와 같은 자리에서 갈린다.
  #
  # 차분 검증(2026-09-09): 파서가 비밀번호로 읽는 주소 120개에 표식을 심어 재니
  # 옛 묶음은 **72개**에서 표식이 그대로 남았고 이 묶음은 **0개**였다.
