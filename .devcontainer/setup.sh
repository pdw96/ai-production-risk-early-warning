...
redact_url() {
  ...
  #
  # 그래서 지금 모양이 이렇다 — 사용자 이름 쪽은 `.*`(탐욕적이라 **마지막 `@`** 를
  # 잡아 파서와 같은 자리에서 가른다), 비밀번호 쪽은 `[^@]*`(첫 `@` 앞에서 멈추므로
  # 질의의 `@` 를 넘어가지 않는다). 자격증명이 아닌 것을 더 가리는 쪽으로 틀릴 수는
  # 있지만, 그 실패는 「로그가 덜 친절하다」이지 「비밀이 샌다」가 아니다.
